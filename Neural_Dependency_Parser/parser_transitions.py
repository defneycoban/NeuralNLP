...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        minibatch = unfinished_parses[:batch_size]  # Take the first batch_size parses
        ...
        # Remove completed parses: empty buffer and stack size is 1 (only ROOT remains)
        unfinished_parses = [parse for parse in unfinished_parses if not (len(parse.buffer) == 0 and len(parse.stack) == 1)]
    
    # Extract dependencies from each parse
