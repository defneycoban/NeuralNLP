...
class ScaledDotAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        unnormalized_attention = torch.bmm(q, k.transpose(1, 2)) * self.scaling_factor  # scaling by 1/sqrt(d)
...
class CausalScaledDotAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        mask = torch.triu(torch.ones(unnormalized_attention.size()), diagonal=1).to(queries.device)
        unnormalized_attention = unnormalized_attention.masked_fill(mask == 1, self.neg_inf)
